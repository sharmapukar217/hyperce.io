'use client';

import { DropdownMenu } from 'radix-ui';
import { ChevronDown } from 'lucide-react';
import { useEffect, useState } from 'react';

import {
  OfficeDetails,
  Domain,
  contactDetailsMapping
} from '@/data/ContactDetailsData/OfficeMapping';

const filteredContacts = Object.values(contactDetailsMapping)
  .filter((details) => details.phoneNumber && details.phoneNumber.length > 0)
  .map((details) => ({
    country: details.country,
    phoneNumber: details.phoneNumber
  }));

export default function WhatsAppBtn() {
  const [whatsAppNumber, setWhatsAppNumber] = useState(
    contactDetailsMapping[Domain.HyperceUs].phoneNumber
  );

  useEffect(() => {
    const domain: Domain = window.location.hostname as Domain;
    if (domain !== Domain.HyperceIo && contactDetailsMapping[domain]) {
      setWhatsAppNumber(contactDetailsMapping[domain].phoneNumber);
    }
  }, []);

  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger
        aria-label="Chat on WhatsApp"
        className="bg-[#2ace67] group fixed bottom-4 w-fit inset-x-4 md:left-8 rounded-xl h-12 p-3 flex justify-start items-center shadow-lg text-white data-[state=open]:rounded-t-[0px] overflow-hidden"
      >
        <svg
          fill="currentColor"
          viewBox="64 64 896 896"
          className="w-6 h-auto shrink-0"
        >
          <path d="M713.5 599.9c-10.9-5.6-65.2-32.2-75.3-35.8-10.1-3.8-17.5-5.6-24.8 5.6-7.4 11.1-28.4 35.8-35 43.3-6.4 7.4-12.9 8.3-23.8 2.8-64.8-32.4-107.3-57.8-150-131.1-11.3-19.5 11.3-18.1 32.4-60.2 3.6-7.4 1.8-13.7-1-19.3-2.8-5.6-24.8-59.8-34-81.9-8.9-21.5-18.1-18.5-24.8-18.9-6.4-.4-13.7-.4-21.1-.4-7.4 0-19.3 2.8-29.4 13.7-10.1 11.1-38.6 37.8-38.6 92s39.5 106.7 44.9 114.1c5.6 7.4 77.7 118.6 188.4 166.5 70 30.2 97.4 32.8 132.4 27.6 21.3-3.2 65.2-26.6 74.3-52.5 9.1-25.8 9.1-47.9 6.4-52.5-2.7-4.9-10.1-7.7-21-13z"></path>
          <path d="M925.2 338.4c-22.6-53.7-55-101.9-96.3-143.3a444.35 444.35 0 00-143.3-96.3C630.6 75.7 572.2 64 512 64h-2c-60.6.3-119.3 12.3-174.5 35.9a445.35 445.35 0 00-142 96.5c-40.9 41.3-73 89.3-95.2 142.8-23 55.4-34.6 114.3-34.3 174.9A449.4 449.4 0 00112 714v152a46 46 0 0046 46h152.1A449.4 449.4 0 00510 960h2.1c59.9 0 118-11.6 172.7-34.3a444.48 444.48 0 00142.8-95.2c41.3-40.9 73.8-88.7 96.5-142 23.6-55.2 35.6-113.9 35.9-174.5.3-60.9-11.5-120-34.8-175.6zm-151.1 438C704 845.8 611 884 512 884h-1.7c-60.3-.3-120.2-15.3-173.1-43.5l-8.4-4.5H188V695.2l-4.5-8.4C155.3 633.9 140.3 574 140 513.7c-.4-99.7 37.7-193.3 107.6-263.8 69.8-70.5 163.1-109.5 262.8-109.9h1.7c50 0 98.5 9.7 144.2 28.9 44.6 18.7 84.6 45.6 119 80 34.3 34.3 61.3 74.4 80 119 19.4 46.2 29.1 95.2 28.9 145.8-.6 99.6-39.7 192.9-110.1 262.7z"></path>
        </svg>
        <div className="inline-flex items-center justify-start gap-2 w-0 overflow-hidden group-hover:w-fit group-hover:ps-2 group-data-[state=open]:w-fit group-data-[state=open]:ps-2 transition-all">
          <span className="font-bold">Chat on WhatsApp</span>
          <ChevronDown className="w-4 h-auto shrink-0 transition-all group-data-[state=open]:rotate-180" />
        </div>
      </DropdownMenu.Trigger>

      <DropdownMenu.Content className="bg-[#2ace67] text-white rounded-t-xl w-0 h-0 opacity-0 pt-3 pb-0.5 data-[state=open]:opacity-100 data-[state=open]:w-[--radix-popper-anchor-width] data-[state=open]:h-auto transition-all">
        {filteredContacts.map((detail) => (
          <DropdownMenu.Group
            key={detail.country}
            className="mb-2 border-b border-[#25b459]"
          >
            <DropdownMenu.Label className="font-semibold px-3">
              {detail.country}
            </DropdownMenu.Label>
            {detail.phoneNumber?.map((phone) => (
              <DropdownMenu.Item
                asChild
                key={phone}
                className="block hover:bg-[#25b459] px-3 py-2"
              >
                <a
                  href={`https://wa.me/${phone.replace(/[+-]/g, '')}`}
                  target="_blank"
                >
                  {phone}
                </a>
              </DropdownMenu.Item>
            ))}
          </DropdownMenu.Group>
        ))}
      </DropdownMenu.Content>
    </DropdownMenu.Root>
  );
}
